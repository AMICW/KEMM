...
- 你准备开始改代码，但还不确定真实实现入口在哪里
- 你需要快速判断某项改动会不会踩到 benchmark / ship / 可视化边界
...
- 3 主目标优化
- 完整报告默认使用 `full_tuned` 场景 solve profile
- quick 报告默认回退到 `legacy_uniform`
- 额外输出分析指标
- 核心论文图包默认开启
...
8. 严格可比实验优先走 `--strict-comparable` 自动注入 `*_matched` 分组，不要手工复制预算参数
9. 如果动 ship 场景生成逻辑，同步保证 `scenario_catalog.json` 还能记录 family / seed / difficulty
10. 如果改了统计口径，同步维护 `raw/statistical_tests.*` 与 `reports/statistical_significance.md` 语义
11. 如果改了扰动扫描口径，同步维护 `raw/robustness_*` 与 `reports/robustness_sweep.md`
